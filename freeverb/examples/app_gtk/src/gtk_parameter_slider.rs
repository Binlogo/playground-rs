use {
    ...
    gtk::prelude::*,
    gtk::{Align, Label, Orientation, PositionType, Scale},
};
...
pub fn make_slider(
    ...
    scale.set_value(value_converter.user_to_linear(parameter.default_user_value()));
    scale.connect_format_value({
        move |_, x| string_converter.to_string(value_converter.linear_to_user(x))
    });

...
impl ParameterProvider for FreeverbModule {
    ...
    fn parameter(id: usize) -> Box<dyn Parameter> {
        match Parameters::from_usize(id).unwrap() {
            ...
            Parameters::Width => Box::new(FloatParameter::new("Width").default_user_value(0.5)),
            Parameters::RoomSize => {
                Box::new(FloatParameter::new("Room Size").default_user_value(0.5))
            }
            Parameters::Freeze => Box::new(BoolParameter::new("Freeze")),
            Parameters::Dry => Box::new(FloatParameter::new("Dry").default_user_value(0.0)),
            Parameters::Wet => Box::new(FloatParameter::new("Wet").default_user_value(1.0)),
        }
    }
}
